...
public class FeedbackController(Context context) : ControllerBase
{
    ...
    public async Task<ActionResult<List<KeyValuePair<FeedbackConditions, double>>>> GetPercentages(string screenId, int daysBack = 7)
    {
        ...
        var feedbackItems = 
            screenId == "all" ? //can append more conditions if needed
                await context.FeedbackConditions.Where(log => log.Time > DateTime.Now.AddDays(-daysBack)).ToListAsync() : //all screens will be selected for the statistics
                await context.FeedbackConditions.Where(log => log.Time > DateTime.Now.AddDays(-daysBack) && log.ScreenId == screenId).ToListAsync(); //only screens matching the screenId will be given
    ...
    public async Task<ActionResult<List<KeyValuePair<FeedbackConditions, double>>>> GetPercentages(string screenId, DateTime startDate, DateTime endDate)
    {
        ...
        var feedbackItems = 
            screenId == "all" ? //can append more conditions if needed
                await context.FeedbackConditions.Where(log => log.Time > startDate && log.Time < endDate).ToListAsync() : //all screens will be selected for the statistics
                await context.FeedbackConditions.Where(log => log.Time > startDate && log.Time < endDate && log.ScreenId == screenId).ToListAsync(); //only screens matching the screenId will be given
